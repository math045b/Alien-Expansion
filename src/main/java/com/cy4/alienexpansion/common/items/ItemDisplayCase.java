...
public class ItemDisplayCase extends HangingEntity {
	private static final Logger PRIVATE_LOGGER = LogManager.getLogger();
	private static final DataParameter<ItemStack> ITEM = EntityDataManager.createKey(ItemFrameEntity.class,
			DataSerializers.ITEMSTACK);
	private static final DataParameter<Integer> ROTATION = EntityDataManager.createKey(ItemFrameEntity.class,
			DataSerializers.VARINT);
	private float itemDropChance = 1.0F;

	public ItemDisplayCase(EntityType<? extends ItemFrameEntity> p_i50224_1_, World p_i50224_2_) {
		super(p_i50224_1_, p_i50224_2_);
	}

	public ItemDisplayCase(World worldIn, BlockPos p_i45852_2_, Direction p_i45852_3_) {
	...
	protected void updateBoundingBox() {
		if (this.facingDirection != null) {
			double d1 = (double) this.hangingPosition.getX() + 0.5D
					- (double) this.facingDirection.getXOffset() * 0.46875D;
			double d2 = (double) this.hangingPosition.getY() + 0.5D
					- (double) this.facingDirection.getYOffset() * 0.46875D;
			double d3 = (double) this.hangingPosition.getZ() + 0.5D
					- (double) this.facingDirection.getZOffset() * 0.46875D;
	...
	public boolean onValidSurface() {
		if (!this.world.hasNoCollisions(this)) {
			...
			BlockState blockstate = this.world
					.getBlockState(this.hangingPosition.offset(this.facingDirection.getOpposite()));
			return blockstate.getMaterial().isSolid()
					|| this.facingDirection.getAxis().isHorizontal() && RedstoneDiodeBlock.isDiode(blockstate)
							? this.world.getEntitiesInAABBexcluding(this, this.getBoundingBox(), IS_HANGING_ENTITY)
									.isEmpty()
							: false;
		}
	}
	...
	public void setDisplayedItemWithUpdate(ItemStack stack, boolean p_174864_2_) {
		if (!stack.isEmpty()) {
			stack = stack.copy();
			stack.setCount(1);
			stack.setItemFrame(new ItemFrameEntity(this.world, this.getPosition(), this.facingDirection));
		}
	...
	public void notifyDataManagerChange(DataParameter<?> key) {
		if (key.equals(ITEM)) {
			ItemStack itemstack = this.getDisplayedItem();
			if (!itemstack.isEmpty() && itemstack.getItemFrame() != new ItemFrameEntity(this.world, this.getPosition(),
					this.facingDirection))
				itemstack.setItemFrame(new ItemFrameEntity(this.world, this.getPosition(), this.facingDirection));
		}
	}
