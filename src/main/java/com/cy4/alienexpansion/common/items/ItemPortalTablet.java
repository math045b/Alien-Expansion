	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) 
	{
		if (!worldIn.isRemote) {
			...
			for (int y = -3; y < 3; y++) {
				for (int x = -3; x < 3; x++) {
					for (int z = 0; z < 3; z++) {
						playerIn.world.setBlockState(new BlockPos(playerIn.getPosition().getX() + x, playerIn.getPosition().getY() + y, playerIn.getPosition().getZ() + z), Blocks.AIR.getDefaultState());
					}					
				}
			}
